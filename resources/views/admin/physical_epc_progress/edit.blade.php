{{-- resources/views/admin/physical_epc_progress/edit.blade.php --}}
<x-app-layout>
    <div class="container py-5">
        <h2 class="mb-4 fw-bold text-primary">
            <i class="fas fa-edit me-2"></i> Edit Physical EPC Progress
        </h2>
        ...
        <form action="{{ route('admin.physical_epc_progress.update', $physicalEpcProgress->id) }}" method="POST" enctype="multipart/form-data" id="progressForm">
            @csrf
            @method('PUT')

            {{-- Hidden sub_package_project_id --}}
            <input type="hidden" name="sub_package_project_id" value="{{ $sub_package_project_id }}">
            ...
            <div class="mb-3">
                ...
                <select name="epcentry_data_id" id="epcentry_data_id" class="form-control" required>
                    <option value="">-- Select Activity --</option>
                    @foreach ($epcEntries as $entry)
                        <option value="{{ $entry->id }}" 
                                data-percent="{{ $entry->percent }}" 
                                data-amount="{{ $entry->amount ?? 0 }}"
                                {{ $entry->id == old('epcentry_data_id', $physicalEpcProgress->epcentry_data_id) ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" name="percent" id="percent" class="form-control" placeholder="Enter percent" value="{{ old('percent', $physicalEpcProgress->percent) }}">
            ...
            {{-- <div class="mb-3">
                ...
                <input type="number" step="0.01" name="amount" id="amount" class="form-control" placeholder="Calculated amount" value="{{ old('amount', $physicalEpcProgress->amount) }}" readonly>
            </div> --}}

            {{-- Items --}}
            <div class="mb-3">
                <label for="items" class="form-label">Items</label>
                <textarea name="items" id="items" rows="3" class="form-control" placeholder="Enter items description">{{ old('items', $physicalEpcProgress->items) }}</textarea>
            </div>

            {{-- Progress Submitted Date --}}
            <div class="mb-3">
                <label for="progress_submitted_date" class="form-label">Progress Submitted Date</label>
                <input type="date" name="progress_submitted_date" id="progress_submitted_date" class="form-control" value="{{ old('progress_submitted_date', optional($physicalEpcProgress->progress_submitted_date)->format('Y-m-d')) }}">
            </div>
            ...
            <a href="{{ route('admin.physical_epc_progress.index', ['sub_package_project_id' => $sub_package_project_id]) }}" class="btn btn-secondary">
                Cancel
            </a>
        </form>
    </div>
    ...
    <script>
        ...
        function updatePercentAndAmountLimits() {
            ...
            // For edit, subtract current record percent from sum:
            let currentSum = parseFloat(progressSums[selectedEntryId] ?? 0);
            const currentRecordPercent = parseFloat("{{ old('percent', $physicalEpcProgress->percent) }}") || 0;
            currentSum -= currentRecordPercent;
