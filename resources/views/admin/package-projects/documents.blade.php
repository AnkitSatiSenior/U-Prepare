<x-app-layout>
    ...
    <style>
        ...
        .month-folder { transition: transform 0.2s; cursor: pointer; }
        .month-folder:hover { transform: translateY(-5px); box-shadow: 0 .5rem 1rem rgba(0,0,0,.15)!important; }
        
        .gallery-img-wrapper { 
            position: relative; overflow: hidden; border-radius: 8px; height: 160px; 
        ...
        .file-icon-wrapper:hover { background: #e9ecef; }
        
        /* Theme Specific Colors */
        .folder-env i { color: #198754 !important; } /* Green for Environment */
        .folder-soc i { color: #0d6efd !important; } /* Blue for Social */
    </style>
            ...
            <div class="card-body p-4">
                ...
                @else
                    ...
                    <div class="month-list row g-4">
                        @php
                            $envMonths = collect($envGallery)->keys()->map(fn($d)=>\Carbon\Carbon::parse($d)->format('Y-m'))->unique()->sortDesc();
                        @endphp
                        @foreach ($envMonths as $month)
                            <div class="col-lg-3 col-md-4 col-sm-6">
                                <div class="card month-folder shadow-sm border h-100 text-center p-4" data-month="{{ $month }}">
                                    ...
                                    <h5 class="fw-bold text-dark mb-1">{{ \Carbon\Carbon::parse($month.'-01')->format('F Y') }}</h5>
        ...
        <div class="card shadow-sm border-0 mb-4" id="soc-gallery-root">
            ...
            <div class="card-body p-4">
                ...
                @else
                    ...
                    <div class="month-list row g-4">
                        @php
                            $socMonths = collect($socGallery)->keys()->map(fn($d)=>\Carbon\Carbon::parse($d)->format('Y-m'))->unique()->sortDesc();
                        @endphp
